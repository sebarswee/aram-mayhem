...
import { LightningFocusEffectPool, LightningFocusEffectConfig } from '@/pools/effects/LightningFocusEffectPool';
...
describe('LightningFocusEffectPool', () => {
  let mockSceneData: ReturnType<typeof createMockScene>;
  let pool: LightningFocusEffectPool;

  beforeEach(() => {
    mockSceneData = createMockScene();
    const { scene } = mockSceneData;
    pool = new LightningFocusEffectPool(scene, 2);
  });
  ...
  describe('基础功能', () => {
    ...
    it('应该能获取和释放对象', () => {
      const config: LightningFocusEffectConfig = {
        x: 100,
        y: 100,
        range: 150,
        duration: 400,
