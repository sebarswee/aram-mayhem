...
import { HolyJudgmentEffectPool, HolyJudgmentEffectConfig } from '@/pools/effects/HolyJudgmentEffectPool';
...
function createMockRectangle(name: string) {
  return {
    x: 0,
    y: 0,
    width: 50,
    ...
    scaleY: 1,
    alpha: 1,
    setSize: vi.fn(function(this: any, w: number, h: number) { this.width = w; this.height = h; return this; }),
  ...
  const scene = {
    ...
    add: {
      ...
      rectangle: vi.fn((x: number, y: number, width: number, height: number, color: number, alpha: number) => {
        const rect = createMockRectangle(`rectangle_${rectangleCounter++}`);
        rect.x = x;
        rect.y = y;
        rect.width = width;
        rect.height = height;
...
describe('HolyJudgmentEffectPool', () => {
  ...
  describe('基础功能', () => {
    ...
    it('应该能获取和释放对象', () => {
      const config: HolyJudgmentEffectConfig = {
        x: 100,
        y: 100,
        targetX: 100,
        targetY: 100,
  ...
  describe('多次获取', () => {
    it('应该支持多次获取不同的效果实例', () => {
      const config1: HolyJudgmentEffectConfig = {
        x: 100,
        y: 100,
        targetX: 100,
        targetY: 100,
      ...
      const config2: HolyJudgmentEffectConfig = {
        x: 200,
        y: 200,
        targetX: 200,
        targetY: 200,
